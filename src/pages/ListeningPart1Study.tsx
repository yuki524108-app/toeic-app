...
import type { AppData, ListeningPart1Item, ProgressRecord } from "../types";
...
const items = listeningPart1Items as ListeningPart1Item[];
const CHOICE_LABELS = ["A", "B", "C", "D"] as const;
const RATES = [0.75, 1, 1.25] as const;

export default function ListeningPart1Study({
  data,
  onAnswer,
  onToggleBookmark,
}: {
  ...
  const queue = useMemo(() => {
    return filterByMode(items, data, mode);
  ...
  async function handlePlay() {
    ...
    await speakSequence(current.choices, rate, {
      pauseMs: 700,
  ...
  if (isDone) {
    return (
      <DoneState correct={sessionCorrect} total={queue.length} label="リスニング Part 1" />
    );
  }
    ...
    <div className="mx-auto flex max-w-md flex-col px-5 pb-28 pt-8">
      ...
      {!ttsSupported && (
        <div className="mt-4 rounded-sm border border-(--color-incorrect) bg-(--color-incorrect-soft) p-3 text-xs leading-relaxed text-(--color-incorrect)">
          ...
          API）に対応していないため、代わりに4つの英文をすべて表示しています。
        </div>
      )}
      ...
      <div className="relative mt-6">
        <button
          ...
          className="absolute right-3 top-3 z-10 rounded-full bg-(--color-paper-raised) p-1.5 shadow-sm"
        >
        ...
        <div className="overflow-hidden rounded-sm border border-(--color-line) bg-(--color-paper-raised)">
          ...
          <div
            className="mt-2 [&>svg]:block [&>svg]:w-full [&>svg]:h-auto"
            dangerouslySetInnerHTML={{ __html: current.image }}
          />
        </div>
      </div>
      ...
      {ttsSupported ? (
        <div className="mt-5 flex flex-col items-center">
          ...
          <p className="mt-3 text-xs text-(--color-muted)">
            {isPlaying
              ? playingSegment !== null
                ? `選択肢 ${CHOICE_LABELS[playingSegment]} を再生中…`
                : "再生中…"
              : hasPlayedOnce
              ? "もう一度再生する"
              : "タップして4つの英文を再生"}
        ...
        <div className="mt-4 space-y-2 rounded-sm border border-(--color-line) bg-(--color-paper-raised) p-4 text-sm leading-relaxed text-(--color-ink)">
          {current.choices.map((c, i) => (
            <p key={i}>
              <span className="font-medium">{CHOICE_LABELS[i]}.</span> {c}
            </p>
          ))}
        </div>
      )}
